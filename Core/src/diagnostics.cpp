...
void gambit_core::backend_diagnostic() {
  bool all_good = true;
  table_formatter table("Backends", "Version", "Path to lib", "Status ",
                        " #func ", "#types ", "#ctors");
  table.padding(1);
  table.capitalize_title();
  ...
  for (const auto &backend : backend_versions) {
    ...
    for (const auto &version : backend.second) {
      ...
      const str path = backendData->path(
          backend.first, version); // Get the path of this backend
      const str status = backend_status(
          backend.first, version, all_good); // Save the status of this backend

      // Count up the number of functions in this version of the backend, using
      // the registered functors.
      for (const auto &functor : backendFunctorList) {
        if (functor->origin() == backend.first and
            functor->version() == version)
          nfuncs++; // If backend matches, increment the count of the functions
                    // in this version
      }

      // Do things specific to versions that provide classes
      if (backendData->classloader.at(backend.first + version)) {
        const std::set<str> classes = backendData->classes.at(
            backend.first + version); // Retrieve classes loaded by this version
        ntypes =
            classes.size(); // Get the number of classes loaded by this backend
        for (const auto &class_ :
             classes) // class is a C++ keyword, so use class_ here which allows
                      // the same readability.
        {
          nctors +=
              backendData->factory_args.at(backend.first + version + class_)
                  .size(); // Add the number of factories for this class to the
                           // total
        }
      }

      // Print the info
      const str firstentry =
          (&version == std::addressof(*backend.second.begin()) ? backend.first
                                                               : "");
      ...
      if (status == "OK")
        ...
        table.red() << status;
      table << " " + std::to_string(nfuncs) << std::to_string(ntypes)
            << std::to_string(nctors);
    }
  }
...
void gambit_core::model_diagnostic() {
  ...
  ModelHierarchy modelGraph(*modelInfo, primaryModelFunctorList, graphfile,
                            false);
  out << endl
      << "Created graphviz model hierarchy graph in " + graphfile + "." << endl;
  out << endl
      << "To get postscript plot of model hierarchy, please run: " << endl;
  out << GAMBIT_DIR << "/Core/scripts/./graphviz.sh " + graphfile << endl;
#else
  out << endl
      << "To get postscript plot of model hierarchy, please install graphviz, "
         "rerun cmake and remake GAMBIT."
      << endl;
#endif
  out << table.str();
...
void gambit_core::capability_diagnostic() {
  ...
  table_formatter table("Capabilities", "Available in (modules/models)",
  ...
  for (const auto &capability : capabilities) {
    ...
    for (const auto &functor : functorList) {
      if (functor->capability() == capability)
        modset.insert(functor->origin());
    }
    std::set<str> beset;
    for (const auto &functor : backendFunctorList) {
      if (functor->capability() == capability)
        beset.insert(functor->origin());
    }

    // Make strings out of the sets
    std::string mods("");
    for (const auto &mod : modset) {
      if (&mod != std::addressof(*modset.begin()))
        mods += ", ";
      mods += mod;
    }
    std::string bes("");
    for (const auto &be : beset) {
      if (&be != std::addressof(*beset.begin()))
        bes += ", ";
      bes += be;
    }

    // Identify the primary model parameters with their models.
    if (mods.empty() and bes.empty())
      mods = capability.substr(0, capability.length() - 11);
...
void gambit_core::scanner_diagnostic() {
  ...
  const std::string output =
      Scanner::Plugins::plugin_info().print_all("scanner");
...
void gambit_core::test_function_diagnostic() {
  const std::string output =
      Scanner::Plugins::plugin_info().print_all("objective");
  print_to_screen(output, "objectives");
}

void gambit_core::prior_diagnostic() {
  const std::string output =
      Scanner::Plugins::plugin_info().print_priors("priors");
...
void gambit_core::ff_prior_diagnostic(const str &command) {
  if (command != "priors") {
    const std::string output =
        Scanner::Plugins::plugin_info().print_priors(command);
...
void gambit_core::ff_module_diagnostic(const str &command) {
  ...
  for (const auto &module : modules) {
    if (command == module) {
      out << "Information for module " << module << "." << std::endl
          << std::endl;
      table_formatter table(
          "", "", "", "LOOP MANAGER:", "DEPENDENCIES / BACKEND REQUIREMENTS");
      table.new_titles("Function", "Capability", "Result Type", " IS  NEEDS",
                       "[type]         {type}");
      table.padding(1);
      table.capitalize_title();
...
void gambit_core::ff_backend_diagnostic(const str &command) {
  ...
  for (const auto &backend : backend_versions) {
    if (command == backend.first) {
      ...
      out << "Information for backend " << backend.first << "." << std::endl
          << std::endl;

      // Loop over all registered versions of this backend
      for (const auto &version : backend.second) {
        bool who_cares;
        const str path = backendData->corrected_path(
            backend.first, version); // Save the path of this backend
        const str status =
            backend_status(backend.first, version,
                           who_cares); // Save the status of this backend
        out << "Version: " << version << std::endl;
        out << "Path to library: " << path << std::endl;
        out << "Library status: " << status << std::endl;

        table_formatter back_table("  Function/Variable", "Capability", "Type",
                                   "Status");
        ...
        if (back_table.rows() > 0)
          out << back_table.str();
        table_formatter class_table("  Class", "Constructor overload",
                                    "Status");
        ...
        if (backendData->classloader.at(backend.first + version)) {
          std::set<str> classes;
          if (backendData->classes.find(backend.first + version) !=
              backendData->classes.end())
            classes = backendData->classes.at(backend.first + version);
          has_classloader = true;
          // Loop over the classes
          for (const auto &class_ :
               classes) // class is a C++ keyword, so use class_ here which
                        // allows the same readability.
          {
            const std::set<str> ctors =
                backendData->factory_args.at(backend.first + version + class_);
            // Loop over the constructors in each class
            for (const auto &ctor : ctors) {
              str args = ctor;
              boost::replace_all(args, "my_ns::", "");
              const str ss = backendData->constructor_status.at(
                  backend.first + version + class_ + args);
              const str firstentry =
                  (&ctor == std::addressof(*ctors.begin()) ? class_ : "");
      ...
      // Tell the user what the default version is for classes of this backend
      // (if there are any).
      if (has_classloader) {
        const std::map<str, str> defs = backendData->default_safe_versions;
        const str my_def = (defs.find(backend.first) != defs.end()
                                ? backendData->version_from_safe_version(
                                      backend.first, defs.at(backend.first))
                                : "none");
        out << std::endl
            << "Default version for loaded classes: " << my_def << std::endl
            << std::endl;
      }
      break;
    }
...
void gambit_core::ff_model_diagnostic(const str &command) {
  ...
  for (const auto &functor : primaryModelFunctorList) {
    ...
    if (command == model) {
      ...
      // Need copies of lineage and descendant vectors with self-reference
      // removed
      std::vector<str> lin_X = mod.lineage;
      std::vector<str> des_X = mod.descendants;

      // Erase element matching name
      lin_X.erase(std::remove(lin_X.begin(), lin_X.end(), mod.name),
                  lin_X.end());
      des_X.erase(std::remove(des_X.begin(), des_X.end(), mod.name),
                  des_X.end());
...
void gambit_core::ff_capability_diagnostic(const str &command) {
  ...
  for (const auto &capability : capabilities) {
    if (command == capability) {
      out << "Information for capability " << capability << "." << std::endl
          << std::endl;

      // Retrieve info on this capability from the database file
      const capability_info cap = get_capability_info(capability);
      std::vector<std::pair<str, std::map<str, std::set<std::pair<str, str>>>>>
          origins;
      origins.push_back(
          std::pair<str, std::map<str, std::set<std::pair<str, str>>>>(
              "modules", cap.modset));
      origins.push_back(
          std::pair<str, std::map<str, std::set<std::pair<str, str>>>>(
              "backends", cap.beset));
        ...
        if (not origin.second.empty()) {
          ...
          for (const auto &module_or_backend : origin.second) {
            ...
            for (const auto &function : module_or_backend.second) {
              ...
              out << "      function " << function.first << " [type "
                  << function.second << "]" << std::endl;
            }
          }
          out << std::endl;
        }
...
void gambit_core::ff_scanner_diagnostic(const str &command) {
  const std::string output =
      Scanner::Plugins::plugin_info().print_plugin("scanner", command);
  print_to_screen(output, command);
}
...
void gambit_core::ff_test_function_diagnostic(const str &command) {
  const std::string output =
      Scanner::Plugins::plugin_info().print_plugin("objective", command);
  print_to_screen(output, command);
}
