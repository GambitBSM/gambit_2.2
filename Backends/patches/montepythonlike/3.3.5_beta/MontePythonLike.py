        ...
        for key, value in dictionary.items():
            ...
            if num_flag is True:
                if array_flag is False:
                    #print("THIS REPLACEMENT IS HIGHLY SUSPICIOUS !! ")
...
class Likelihood_sd(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        # Tell class that for sure we are going to need sd
        self.need_cosmo_arguments(
        ...
        """The user can either pass an external noise file (useful for FIRAS), or compute it here automatically (will be done for PIXIE)."""
        ...
        if self.noise_from_file:
            ...
            if hasattr(self, 'detector_bin_number') or hasattr(self, 'detector_nu_delta') or hasattr(self, 'detector_nu_min') or hasattr(self, 'detector_nu_max'):
                print ' /|\  Warning! You asked to read the noise from a file, but you also passed detector spcifications.'
                print '/_o_\ I will ignore the bin_number, nu_delta, nu_min, and nu_max you passed in the likelihood file, and calculate them from the noise file.'

            # This is because class only needs the name, but knows the directory, while here we need to calculate it properly
        ...
        # Compute noise for mission based on detector specifications
            ...
            # in the future we will add more options, once we know how to get delta_Ic from delta_I^{I,Q,U}
        ...
        # Now we pass things to class. Class wil take either detctor name & noise file, or detector name & specifications
        if self.noise_from_file:
            self.need_cosmo_arguments(data, {'sd_detector_name':self.detector})
            self.need_cosmo_arguments(data, {'sd_detector_file_name':self.noise_file_name})
    ...
    def loglkl(self, cosmo, data):
        ...
        # get SDs from the cosmological code (returned in Jy/sr = 10^(-26)W/m^2/sr/Hz units)
    ...
    def compute_lkl(self, sd, cosmo, data):
        ...
        # Define dimensionless frequency and overall normalisation
        ...
        #5) Integrated CO emission
        A_CO     = data.mcmc_parameters['sd_A_CO']['current'] * data.mcmc_parameters['sd_A_CO']['scale'] # Unitless
        # The equation is taken from Tab. 1 of Abitbol+2016 [1705.01534]
        ...
        #6) Reionization
        y_reio   = data.mcmc_parameters['sd_y_reio_nuisance']['current'] * data.mcmc_parameters['sd_y_reio_nuisance']['scale'] # Unitless
        ...
        #7) Mu parameter (using a small nuisance parameter instead of full CLASS, just for testing purposes!!!)
        ...
        # Calculate data, theory, and noise
        for nu_i in range(self.detector_bin_number):
