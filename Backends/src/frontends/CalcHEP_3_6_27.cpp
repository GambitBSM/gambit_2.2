...
BE_INI_FUNCTION
{
  ...
  if (scan_level)
  {
    ...
    if (ModelInUse("SingletDM"))
    {
      ...
      One_Time_Spec = *Dep::SingletDM_spectrum;
    }
    ...
    // Set Standard Model Variables (i.e. unchanging ones)
    const DecayTable& SM_decays = *Dep::SM_decay_rates;
    Assign_Standard_Model_Variables(One_Time_Spec, SM_decays);
  }
  ...
  {
    // Model contents
    static const SpectrumContents::MSSM MSSM_contents;

    // Parameter list
    static const std::vector<SpectrumParameter> MSSM_params = MSSM_contents.all_parameters();

    const Spectrum& spec = *Dep::MSSM_spectrum;

    Assign_All_Values(spec,MSSM_params);
  }
}
END_BE_INI_FUNCTION
...
BE_NAMESPACE
{
  ...
  /// Assigns all SM variables, just once at the start of a scan, which are unchanging and not used as
  /// nuisance parameters. This ensures CalcHEP is using the same values for e.g. fundamental constants as GAMBIT.
  void Assign_Standard_Model_Variables(const Spectrum& spec, const DecayTable& SM_decays)
  {
    // Create SMInputs struct from spectrum
    const SMInputs& sminputs = spec.get_SMInputs();

    // First, some constants. These should always have the correct names if generated by SARAH.
    Assign_Value((char*)"Gf", sminputs.GF);                      // Fermi
    Assign_Value((char*)"aS", sminputs.alphaS);                  // Strong coupling
    ...
    // Set W+ from spectrum - not tree level relation. This changes e.g. sin2W.
    Assign_Value(pdg2mass(24), spec.get(Par::Pole_Mass, "W+"));

    // Assign all SM particle widths to CalcHEP. (Leptons, gauge bosons, heavy quarks.)
    for (auto it = SM_decays.particles.begin(); it != SM_decays.particles.end(); ++it)
    {
      Assign_Value(pdg2width(it->first.first), SM_decays.at(it->first).width_in_GeV);
    }
  }
  ...
  void Assign_All_Values(const Spectrum& spec, std::vector<SpectrumParameter> params)
  {
    ...
    for (auto it = params.begin(); it != params.end(); ++it)
    {
      ...
      {
        if (it->name() == "lambda_h") {} // Do nothing. CalcHEP computes this internally, from v and mH.

        // If the parameter is called lambda_something -> change name to fit with CalcHEP before Assign_Value(),
        // as CalcHEP variables names cannot be more than 6 characters long. (lambda_X -> l_X)
        else if (std::regex_match(it->name(), std::regex("(lambda)(.*)")))
        {
          str chepstring = std::regex_replace(it->name(), std::regex("lambda"), "l");
          char *chepchar = const_cast<char*> (chepstring.c_str());
          Assign_Value(chepchar, spec.get(Par::dimensionless, it->name()));
        }

        else if (it->name() == "tanbeta") { Assign_Value((char*)"betaH", atan(spec.get(Par::dimensionless, it->name()))); }
      }
    ...
    // Assign commonly used nuisance/precision parameters
  ...
  /// Provides spin-averaged decay width for 2 body decay process in CM frame.
  ...
  /// Computes annihilation cross-section for 2->2 process, DM+DMbar -> X + Y. Coannihilations not
  /// currently supported; we require the mass of both in states are equal.
