...
def write_spectrumcontents(gambit_model_name, model_parameters):
  ...
  filename = "SpectrumContents/" + gambit_model_name
  module = "Models"
...
def write_subspectrum_wrapper(gambit_model_name, model_parameters):
  ...
  for i in np.arange(len(sizes)):
    ...
    towrite += (
            ...
            "\n"
             ...
             ",".join(str(j) for j in np.arange(int(sizes[i]))))
  ...
  filename = "SimpleSpectra/" + modelSS
  module = "Models"
  contents = indent(towrite)
  
  write_file(filename, module, contents, header=True)
...
def add_to_registered_spectra(gambit_model):
  ...
  x = 0 # Position of last entry in RegisteredSpectra
  ...
  amend_file(filename, module, newentry, x, header=True)
