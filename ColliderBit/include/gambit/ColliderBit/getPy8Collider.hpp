...
namespace Gambit
{
  ...
  namespace ColliderBit
  {
    ...
    {                                                                  \
      using namespace Pipes::NAME;                                     \
      str SLHA_filename = *Dep::SLHAFileName;                          \
      if (*Loop::iteration >= COLLIDER_FINALIZE && SLHA_filename.empty()) \
      {                                                                \
        piped_invalid_point.request("Got empty SLHA filename. Will invalidate point."); \
      }                                                                \
      getPy8Collider_SLHA(result, *Dep::RunMC, SLHA_filename,          \
       #MODEL_EXTENSION, *Loop::iteration,                             \
