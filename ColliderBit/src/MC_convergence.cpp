      ...
      if (not converged)
      {
        ...
        for (auto& analysis_pointer_pair : ac.get_current_analyses_map())
        {
          ...
          for (auto& sr : analysis_pointer_pair.second->get_results())
          {
            ...
            SR_converged = (_settings->stop_at_sys and total_counts > 0 and absolute_stat_uncert <= sr.n_signal_MC_sys) or
            ...
            if (not SR_converged) all_SR_converged = false;
              ...
              cerr << "DEBUG: absolute_stat_uncert vs sys: " << absolute_stat_uncert << " vs " << sr.n_signal_MC_sys << endl;
